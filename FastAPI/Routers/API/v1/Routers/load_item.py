...
_positive_cache        = TTLCache(ttl=3600, max_entries=512)  # 1 sa - içerik detayı (yayın bilgisi) sık değişmiyor
...
async def _get_plugin_semaphore(plugin_name: str) -> asyncio.Semaphore:
    async with _load_item_sem_lock:
        if plugin_name not in _load_item_semaphores:
            if len(_load_item_semaphores) >= 64:
                _load_item_semaphores.pop(next(iter(_load_item_semaphores)), None)
